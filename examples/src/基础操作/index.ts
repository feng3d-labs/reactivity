...
{
    feng3dResults.push(基础Ref操作(ref, count));
    feng3dBeforeResults.push(基础Ref操作(beforeRef, count));
    vueResults.push(基础Ref操作(vueRef, count));
}
...
// 显示测试代码
document.getElementById('test-code')!.textContent = `基础Ref操作(ref, ${count}); // 运行 ${runs} 次\n\n` + 基础Ref操作.toString();
