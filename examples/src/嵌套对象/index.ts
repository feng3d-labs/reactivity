...
{
    feng3dResults.push(嵌套对象Computed(reactive as any, computed as any, count));
    feng3dBeforeResults.push(嵌套对象Computed(beforeReactive as any, beforeComputed as any, count));
    vueResults.push(嵌套对象Computed(vueReactive as any, vueComputed as any, count));
}
...
document.getElementById('test-code')!.textContent = `嵌套对象Computed(reactive, computed, ${count}); // 运行 ${runs} 次\n\n` + 嵌套对象Computed.toString();
